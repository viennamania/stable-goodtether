  ...
  */
    ...
    const acceptBuyOrder = (
      ...
      smsNumber: string,
        ...
        .then(data => {
            ...
            fetch('/api/payment/getAllPaymentsByStorecode', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    */
    ...
    const cancelTrade = async (orderId: string, index: number) => {
      ...
      if (data.result) {
        ...
        await fetch('/api/payment/getAllPaymentsByStorecode', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  useEffect(() => {
    ...
    const fetchBuyOrders = async () => {
      ...
      const response = await fetch('/api/payment/getAllPaymentsByStorecode', {
          method: 'POST',
          headers: {
              'Content-Type': 'application/json',
          },
      ...
      */
      ...
      // const [buyOrders, setBuyOrders] = useState<BuyOrder[]>([]);
      /*
      let tmpBuyOrders: BuyOrder[] = [];


      data.result.forEach((item: any) => {
        buyOrders.push({
          storecode: item.order.storecode,
          walletAddress: item.order.walletAddress,
          usdtAmount: item.order.usdtAmount,
          krwAmount: item.order.krwAmount,
                              ...
                              <div className="flex mr-2 text-right">
                                {
                                  ...
                                  item.status === 'paymentConfirmed' ? (
                                    ...
                                    <input
                                      ...
                                      className="w-20 h-8 rounded-md text-right text-sm text-zinc-500 font-semibold bg-zinc-100 border border-zinc-300 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                    />
                                  ) : (
                                    <></>
                                  )
